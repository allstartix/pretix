...
class UpdateSettingsForm(SettingsForm):
    update_check_perform = forms.BooleanField(
        ...
        help_text=_("During the update check, AllstarTix will report an anonymous, unique installation ID, "
                    ...
                    "inactive events in your installation to servers operated by our developers. We "
    ...
    update_check_email = forms.EmailField(
        ...
        help_text=_("We will notify you at this address if we detect that a new update is available. This "
                    "address will not be transmitted to on network, the emails will be generated by this server "
                    "locally.")
    )
...
class LicenseCheckForm(forms.Form):
    ...
    poweredby_url = forms.URLField(
        ...
        help_text=_('If you used the previous option, you can set an URL to link in the footer.'),
    )
    source_notice = forms.CharField(
        ...
        help_text=_('If you use AllstarTix under AGPLv3 terms, describe exactly how to download the current source code '
                    'of the site including all modifications and installed plugins. This will be globallly available. '
                    'We make sure to keep it up to date!'),
    )
