...
class LicenseCheckView(StaffMemberRequiredMixin, FormView):
    ...
    def _check_results(self, input):
        ...
        if input.get('base_license') == 'agpl_addperm' and input.get('usage') != 'internally':
            res.append((
                ...
                _('You are in violation of the license. If you\'re not sure whether you qualify for the additional '
                  ...
                  'AGPLv3 terms or obtain a Enterprise license.')
            ))
        ...
        if input.get('base_license') == 'agpl' and not input.get('source_notice'):
            res.append((
                'danger', 'exclamation-circle',
                _('If you\'re using AllstarTix under AGPL license, you need to provide instructions on how to access the '
                  'source code.')
            ))
        ...
        if input.get('base_license') == 'agpl' and input.get('plugins_enterprise'):
            res.append((
                'danger', 'exclamation-circle',
                _('You must not use AllstarTix under AGPL terms if you use pretix Enterprise plugins.')
            ))
            ...
            if input.get('base_changes') == 'yes':
                res.append((
                    ...
                    _('You need to make all changes you made to AllstarTix\' source code freely available to every visitor '
                      'of your site in source code form under the same license terms (AGPLv3 + additional '
        ...
        for entry_point in metadata.entry_points(group='pretix.plugin'):
            ...
            if not input.get('plugins_enterprise') and 'pretix Enterprise' in license:
                res.append((
                    'danger', 'exclamation-circle',
                    _('You selected that you have no active Enterprise licenses, but we found the following '
